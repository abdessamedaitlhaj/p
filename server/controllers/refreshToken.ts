...
export const handleRefreshToken = (req: FastifyRequest, reply: FastifyReply) => {
        ...
        `SELECT u.id, u.username, u.email, u.avatarurl, u.status, u.password, ua.alias
          ...
          (err: any, current_user: User) => {
            ...
            jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET as string, async (err: any, user: any) => {
              if (err || user.username !== current_user.username) return reply.status(403).send();
              try { await touchSession(refreshToken); } catch {}
              const accessToken = generateAccessToken(current_user);
              const { refreshToken: _rt, password: _pass, ...neat_user } = current_user as any;
              console.log("/////////////////////////////++++++")
