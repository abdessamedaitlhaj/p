...
import { createFriendRequest, getFriendInvitations, AllreadyRequested, removeFriendship} from '../models/Friendships';
...
export const getUserById = async(req: FastifyRequest<{Params: Params}>, reply:FastifyReply) => {
	try {
		...
		if (!current_user) {
		  return reply.status(404).send({ message: `User with id ${id} not found` })
		}
...
export const getUsers = async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
			...
			const sanitized = users.map((u: any) => {
				...
				let inMatch = false; let inTournament = false;
				try {
...
export const newAlias = async(req: FastifyRequest, reply:FastifyReply) => {
	...
	try {
		...
		try { tournamentManager.updateAlias(String(req.user_infos.id), alias); } catch {}
	...
	} catch (err:Error | any)
	{
		...
		if (err.message && err.message.includes('UNIQUE constraint failed: userAliases.alias')) {
			return reply.status(400).send({message: 'This alias is already taken. Please choose a different one.'})
		}
...
// decline , setDate
// accept  , setDate
export const NewFriendRequest = async(req: FastifyRequest, reply:FastifyReply) => {
	const { recvId } = req.body as {recvId:string}
	const { id : forwId} = req.query as {id:string}

 	if (!forwId || !recvId){
		return reply.status(400).send({message:'forwId and recvId are required'})
	}
	...
	try {
		...
		if (!user_1 || !user_2){
			return reply.status(404).send({message:'One of the users does not exist'})
		}
		if (forwId === recvId){
		...
		if (alreadyReceived){
			return reply.status(400).send({message:'User has already sent you a friend request'})
		}
		
		const friendships = await createFriendRequest(forwId, recvId)
		return reply.status(201).send({message:'Friend request sent',  friendships})
	} catch (err:Error | any)
	{
...
export const RemoveRequst = async(req: FastifyRequest, reply:FastifyReply) => {
	const {requesterId} = req.body as {requesterId:string}
	const { id : meinId} = req.params as {id:string}

	if (!meinId || !requesterId){
		return reply.status(400).send({message:'meinId and requesterId are required'})
	}
	...
	if (meinId === requesterId){
		return reply.status(400).send({message:'You cannot remove a friend request to yourself'})
	}
...
export const FriendRequests =  async (req: FastifyRequest, reply: FastifyReply) => {
...
export const SearchRequest = async (req: FastifyRequest, reply: FastifyReply) => {
