...
import  {findByEmailOrUsername, createUser, FindByUser}  from '../models/Users.ts';
import { createSession, deleteSessionByToken, findSessionByToken } from '../models/UserSessions.ts';
...
export const Login = async (req: FastifyRequest<{ Body: LoginBody }>, reply: FastifyReply) => {
	...
	try {
		...
		if (!match) return reply.status(401).send({message:'incorrect password'});
		...
		const { password:_pass, refreshToken: _rt, ...neat_user } = findUser as any;
		return reply.status(200).send({ user: {...neat_user}, accessToken, session: { refreshToken }});
