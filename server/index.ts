...
export async function createServer() {
  ...
  import('./activityManager').then(m => { try { m.activityManager.forceUnlockAll(false); console.log('🧹 Cleared stale match/invite locks at startup'); } catch {} });
  });
  ...
  app.get('/api/stats/player/daily7', { preHandler: verifyToken }, (req, reply) => {
  ...
  app.get('/api/stats/matches', { preHandler: verifyToken }, async (_req, reply) => {
    return new Promise((resolve) => {
      db.all('SELECT * FROM game_results', [], (err, rows) => {
        if (err) { reply.code(500).send({ error: 'db error' }); resolve(null); return; }
        reply.send(rows); resolve(null);
      });
    });
  });
  app.get('/api/stats/tournaments', { preHandler: verifyToken }, async (req, reply) => {
    ...
    return new Promise((resolve) => {
      db.all('SELECT * FROM tournaments', [], (err, rows) => {
        ...
        const sanitizedTournaments = userTournaments.map((tournament: any) => {
          try {
            const tournamentData = JSON.parse(tournament.data_json);
            ...
            return {
              ...
              created_by: tournament.created_by === authenticatedUserId ? tournament.created_by : 'other', // Hide other creators
              ...
              player_count: tournamentData.players ? tournamentData.players.length : 0,
              user_participated: tournamentData.players ? tournamentData.players.includes(authenticatedUserId) : false
            };
          } catch (e) {
            console.error('Failed to parse tournament data for sanitization:', e);
            return {
              id: tournament.id,
              name: tournament.name,
              status: tournament.status,
...
const startServer = async () => {
  ...
  try {
    ...
    if (err?.code === 'EADDRINUSE') {
      console.error("🔥 Port already in use ("+port+"). Listing processes holding it:");
      try {
